...
function HeaderAction({ action }: { action: AccountAction }) {
  ...
  return <button type="button" className={className}>{action.label}</button>
}
    ...
    <section className="min-h-screen bg-primary">
      <div className="desktop-frame flex flex-col gap-6 py-5 sm:py-8 lg:gap-6 lg:py-10">
          ...
          {tabs.map((tab) => (
            <Link
              ...
              className={`px-3 py-2.5 text-center font-sans text-[13px] font-semibold transition-colors duration-200 ${
                tab.key === activeTab ? 'bg-gold text-nav' : 'bg-surface text-text-primary'
              }`}
            >
              {tab.label}
            </Link>
        ...
        <div className="flex gap-6">
          <Sidebar activeTab={activeTab} displayName={displayName} email={email} />
